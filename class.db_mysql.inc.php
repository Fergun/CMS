<?php
  ...
  * any later version.                                                       *
  ...
  /* $Id: class.db_mysql.inc.php,v 1.30.2.3 2003/03/13 17:53:17 ralfbecker Exp $ */
	...
	class db extends DatabaseGlobal
	{
		...
		function connect($Database = '', $Host = '', $User = '', $Password = '')
		{
			...
			if ( 0 == $this->Link_ID )
			{
				...
				if (!$this->Link_ID)
				{
					$this->halt(($this->Persistent?'p':'')."connect($Host, $User, \$Password) failed.");
					return 0;
				}
		...
		function query($Query_String, $line = '', $file = '')
		{
			...
			*/
			...
			$Query_String = $this->encoding($Query_String, 1);
		...
		{
      $return_value = $this->Record[$Name];

			$return_value = $this->encoding($return_value);
...
//			$this->haltmsg($msg);
			...
			$info = $debugBacktrace[count($debugBacktrace) - 1];
